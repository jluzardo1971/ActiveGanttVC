...
IMPLEMENT_DYNAMIC(fSTL_MSP2010, CDialog)

fSTL_MSP2010::fSTL_MSP2010(CWnd* pParent /*=NULL*/)
	: CDialog(fSTL_MSP2010::IDD, pParent)
{
	mp_sFontName = _T("Tahoma");
}

fSTL_MSP2010::~fSTL_MSP2010()
{
}

void fSTL_MSP2010::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(fSTL_MSP2010, CDialog)
	ON_WM_SIZE()
END_MESSAGE_MAP()
...
BOOL fSTL_MSP2010::OnInitDialog()
{
	...
	oStyle.SetStartGradientColor(FromArgb(255, 197, 206, 216));
	oStyle.SetEndGradientColor(CLR_WHITE);
	...
	oStyle.SetAppearance(SA_FLAT);
	oStyle.SetBackgroundMode(FP_SOLID);
	oStyle.SetBackColor(FromArgb(255, 199, 200, 255));
	oStyle.SetBorderStyle(SBR_SINGLE);
	...
	oStyle.GetPredecessorStyle().SetLineColor(FromArgb(255, 180, 194, 209));
	oStyle.GetMilestoneStyle().SetShapeIndex(FT_DIAMOND);
    oStyle.GetMilestoneStyle().SetFillColor(FromArgb(255, 199, 200, 255));
    oStyle.GetMilestoneStyle().SetBorderColor(FromArgb(255, 148, 152, 179));
	...
	ActiveGanttVCCtl1.GetTimeLineScrollBar().GetScrollBar().GetThumbButton().SetNormalStyleIndex(_T("ThumbButton"));
	ActiveGanttVCCtl1.GetTimeLineScrollBar().GetScrollBar().GetThumbButton().SetPressedStyleIndex(_T("ThumbButton"));
	ActiveGanttVCCtl1.GetTimeLineScrollBar().GetScrollBar().GetThumbButton().SetDisabledStyleIndex(_T("ThumbButton"));
	...
	for (i = 1; i <= 100; i++)
	{
		...
		oRow.GetNode().SetStyleIndex(_T("NodeStyle"));
	...
	ActiveGanttVCCtl1.GetTasks().Add(_T(""), _T("K1"), GetDateTime(2013, 3, 1, 0, 0, 0), GetDateTime(2014, 3, 1, 0, 0, 0), _T(""), _T("SummaryStyle"), _T("0"));

    ActiveGanttVCCtl1.GetTasks().Add(_T(""), _T("K2"), GetDateTime(2013, 3, 1, 0, 0, 0), GetDateTime(2014, 3, 1, 0, 0, 0), _T(""), _T("TaskStyle"), _T("0"));

	return TRUE; 
}


void fSTL_MSP2010::OnSize(UINT nType, int cx, int cy)
{
