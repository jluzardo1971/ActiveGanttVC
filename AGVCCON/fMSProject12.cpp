...
using namespace MSP2007;

IMPLEMENT_DYNAMIC(fMSProject12, CDialog)

fMSProject12::fMSProject12(CWnd* pParent /*=NULL*/)
	: CDialog(fMSProject12::IDD, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
	mp_sFontName = _T("Tahoma");
}

fMSProject12::~fMSProject12()
{
}

void fMSProject12::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_ACTIVEGANTTVCCTL1, ActiveGanttVCCtl1);
	DDX_Control(pDX, IDC_MSP2007CTRL1, MSP2007Ctl1);
}


BEGIN_MESSAGE_MAP(fMSProject12, CDialog)
	ON_WM_CREATE()
	ON_WM_SIZE()
...
void fMSProject12::ToolBar_Click(UINT nID)
{
...
int fMSProject12::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
BOOL fMSProject12::OnInitDialog()
{
...
void fMSProject12::OnSize(UINT nType, int cx, int cy)
{
	CDialog::OnSize(nType, cx, cy);
	g_Resize(this, &ActiveGanttVCCtl1);
}

void fMSProject12::InitializeAG()
{
...
void fMSProject12::MP12_To_AG()
{
	MSP2007::CMP12 oMP12;
	oMP12 = MSP2007Ctl1.GetMP12();
	CclsTask oAGTask;
	CclsRow oAGRow;
	MSP2007::CTask oMPTask;
	int i;
	int j;
	
	for (i = 1; i <= oMP12.GetoTasks().GetCount(); i++)
	{
		oMPTask = oMP12.GetoTasks().Item(CStr(i));
	...
	for (i = 1; i <= oMP12.GetoTasks().GetCount(); i++)
	{
		...
		{
			MSP2007::CTaskPredecessorLink oMPPredecessor;
			oMPPredecessor = oMPTask.GetoPredecessorLink_C().Item(CStr(j));
			ActiveGanttVCCtl1.GetPredecessors().Add(_T("K") + CStr(oMPTask.GetlUID()), _T("K") + CStr(oMPPredecessor.GetlPredecessorUID()), GetAGPredecessorType((MSP2007::E_TYPE_5)oMPPredecessor.GetyType()), _T(""), _T("T1"));
		}
	}


    // Assignments
	for (i = 1; i <= oMP12.GetoAssignments().GetCount(); i++)
	{
		MSP2007::CAssignment oAssignment;
		oAssignment = oMP12.GetoAssignments().Item(CStr(i));
		...
		if (oAGTask.GetStartDate() != oAGTask.GetEndDate())
		{
			if (oAssignment.GetlResourceUID() > 0)
			{
				...
				{
					oAGTask.SetText(oMP12.GetoResources().Item(_T("K") + CStr(oAssignment.GetlResourceUID())).GetsName());
				}
				else
				{
					oAGTask.SetText(oAGTask.GetText() + _T(", ") + oMP12.GetoResources().Item(_T("K") + CStr(oAssignment.GetlResourceUID())).GetsName());
				}
			}
		}
	}
...
ActiveGanttVC::E_CONSTRAINTTYPE fMSProject12::GetAGPredecessorType(MSP2007::E_TYPE_5 MPPredecessorType)
{
...
void fMSProject12::AGSetStartDate(COleDateTime dtDate)
{
	int i;
...
void fMSProject12::cmdLoadXML()
{
	...
	if (oDialog.DoModal() == IDOK)
	{
		...
		MSP2007Ctl1.Clear();
        MSP2007Ctl1.GetMP12().ReadXML(sFilePath);
		oWaitCursor.Restore();
        InitializeAG();
        MP12_To_AG();
        ActiveGanttVCCtl1.Redraw();
...
void fMSProject12::cmdPrint_Click()
{
	if (ActiveGanttVCCtl1.GetPrinterErrorMessage().GetLength() == 0)
	{
...
void fMSProject12::cmdIndent_Click()
{
}

void fMSProject12::cmdZoomIn_Click()
{
...
void fMSProject12::cmdZoomOut_Click()
{
...
void fMSProject12::mnuLoadXML_Click()
{
	cmdLoadXML();
}

void fMSProject12::mnuSaveXML_Click()
{
	cmdSaveXML();
}

void fMSProject12::mnuPrint_Click()
{
	cmdPrint_Click();
}

void fMSProject12::mnuClose_Click()
{
	CDialog::EndDialog(IDOK);
}


BEGIN_EVENTSINK_MAP(fMSProject12, CDialog)
	ON_EVENT(fMSProject12, IDC_ACTIVEGANTTVCCTL1, 13, fMSProject12::CustomTierDrawActiveganttvcctl1, VTS_DISPATCH)
	ON_EVENT(fMSProject12, IDC_ACTIVEGANTTVCCTL1, 35, fMSProject12::ControlMouseWheelActiveganttvcctl1, VTS_DISPATCH)
	ON_EVENT(fMSProject12, IDC_ACTIVEGANTTVCCTL1, 42, fMSProject12::PagePrintActiveganttvcctl1, VTS_DISPATCH)
END_EVENTSINK_MAP()

void fMSProject12::CustomTierDrawActiveganttvcctl1(LPDISPATCH e)
{
...
void fMSProject12::ControlMouseWheelActiveganttvcctl1(LPDISPATCH e)
{
...
void fMSProject12::PagePrintActiveganttvcctl1(LPDISPATCH e)
{
        ...
        ULONG oColor;
        LONG X2;
        oColor = FromArgb(255, 197, 206, 216);
